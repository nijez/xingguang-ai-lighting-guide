...
metadata: {"openclaw":{"emoji":"💡","version":"4.0.8","date":"2026-08-06","author":"小馨","company":"深圳市馨光智能物联有限公司"}}
---

# 馨光智能灯控制服务 v4.0.8(本地部署版)
...
2. 播放：`xinguang-show <秀名> --room <房间名>`；停止：`xinguang-show --stop`；进度：`xinguang-show --status`
3. **房间名必须取自用户表述**（如"门市来一场传统色秀"→ `--room 门市`）；用户没说房间，必须先问，禁止猜。

### 创作新秀（逐条执行）

1. **范围有歧义必须先问**（例：用户说"金庸灯光秀"→ 先问"14 部全做，还是精选几部经典？"），禁止代用户拍板。
2. 每景一段文案（40–80 字，贴合用户主题、口语可播报）＋两组色对，配色硬标准（引擎会硬校验，不合格整景拒播）：
   - 主色对与辅色对的第一色：色相差 ≥ 60°，或明度差 ≥ 0.4；
   - 每组色对内部两色：RGB 距离 ≥ 60（渐变必须看得出来）。
...
#秀名	（显示名）
（文案一）	#RRGGBB,#RRGGBB	#RRGGBB,#RRGGBB
（文案二）	#RRGGBB,#RRGGBB	#RRGGBB,#RRGGBB
SHOW
```
