...
metadata: {"openclaw":{"emoji":"💡","version":"4.0.17","date":"2026-08-12","author":"小馨","company":"深圳市馨光智能物联有限公司"}}
---

# 馨光智能灯控制服务 v4.0.17(本地部署版)
...
- ✅ **浓度校准（研发授权的唯一 prop 写例外）：每次 `/api/generate` 调用成功后，立即对该灯直连 miloco 后端置档（内嵌于下方并发模板；**不可用 miloco-cli 写 4.106**——其本地规格校验不含 4.X 会拒发，服务端直连才可写入）：白光亮度等级默认置 3；**用户明确指定等级时（如「白光亮度等级调为 2」）以用户值为准（1-10），把模板里的 value 3 替换为用户值**。
- ⚠️ **术语澄清（研发定义，防混淆）：`prop.4.106 brightnesslevel` 是「白光亮度等级」（1-10，馨光健康淡彩光参数，灯膜调 2-3 观感最佳）；`prop.2.2 brightness` 是「整灯亮度」（1-100）。用户说"白光亮度等级"一律指 4.106，严禁误操作 2.2。**。此命令失败不影响换色结果，不重试不报错。除此之外，任何其他 prop 写操作（含 4.X 全部属性）仍然一律禁止。**
...
识别到触发词时，**禁止浏览设备目录或推理设备类型**，直接执行「七、查询功能」中「2. 查询设备」的固定命令，从输出中取该区域内 `model=wainft.light.rgbcwy` 且 `online` 为 `True` 的设备清单；随后按既有铁律**逐台判定**：回读、跳过关灯设备；通过判定的灯按下方「多灯并发下发（固定模板）」一次性下发，下发后逐台回读确认。

自然语言表述（如「门市来个圣诞氛围」）保留为等效兜底路径，规则相同：禁止浏览设备目录或推理设备类型，直接执行上述固定命令，并按既有铁律完成回读、跳过关灯设备、按固定模板并发 generate、回读确认。
...
- **亮度保底规则：** `/api/generate` 的 brightness 默认使用 100（渐变引擎自身控制观感），除非用户明确要求昏暗/夜灯类效果；浅粉、浅蓝等低饱和浅色在部分灯体上渲染偏暗，此类场景务必保持 brightness 100，靠色彩本身表达柔和，不得自行调低亮度。
...
3. **房间/灯的范围必须取自用户表述**；用户没说范围时必须先问（例：「在哪个房间播？还是全屋/只用某盏灯？」），禁止猜。
...
## 十三、灯膜 prop.4.x 属性速查（研发文档，只读参考）

> 本表帮助你解读用户提问与回读数值。**写入权限不变：全部 4.x 中仅 prop.4.106 可写（见铁律），其余一律只读。** 🔒=只写/notify 属性，无法读取。设备不存在的 iid 查询会返回「属性不存在」，属正常。
...
**运行模式与亮度**：4.37 led-mode 运行模式(0=音乐律动,1=静态,2=动态)｜4.58/4.59/4.60 静态/动态/音乐律动亮度(各1-100)｜**4.106 brightnesslevel 白光亮度等级(1-10,唯一可写)**｜4.36 声光律动动态效果(0-50)｜4.35 灵敏度(0-1200)｜4.68 律动放大系数(1-10)｜4.113 音乐律动同步开关
