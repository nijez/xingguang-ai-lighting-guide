...
# One-shot OpenClaw + Xiaomi Miloco installer for a fresh Ubuntu cloud VM.
...
SCRIPT_VERSION="2026-06-23.9"
TOTAL_STEPS=6
MILOCO_VERSION="${MILOCO_VERSION:-2026.6.18}"
OPENCLAW_PORT="${OPENCLAW_PORT:-18789}"
OPENCLAW_BIND="${OPENCLAW_BIND:-loopback}"
RUN_SYSTEM_UPGRADE="${RUN_SYSTEM_UPGRADE:-1}"
OPENCLAW_UPDATE="${OPENCLAW_UPDATE:-1}"
...
run_openclaw_installer() {
  ...
  if [[ -n "$registry" ]]; then
    ...
    if ! (curl -fsSL https://openclaw.ai/install.sh | env \
      OPENCLAW_NO_PROMPT=1 \
      OPENCLAW_NO_ONBOARD=1 \
      OPENCLAW_INSTALL_METHOD=npm \
      npm_config_registry="$registry" \
      bash -s -- --no-onboard --no-prompt --install-method npm); then
      log "OpenClaw install/update failed with npm mirror; retrying official npm registry"
...
wait_for_openclaw_gateway() {
  ...
  for attempt in {1..20}; do
    if openclaw gateway status >"$status_file" 2>&1 &&
...
verify_install() {
  ...
  openclaw gateway status | sed -n '1,45p' || true
...
请选择操作:
  1) 一键傻瓜式部署
     系统更新 -> OpenClaw 升级 -> Miloco 2.0 -> 平台/米家绑定提示
...
run_full_deploy() {
  ...
  step_start="$(date +%s)"
  step_start_msg 1 "Update Ubuntu system and base packages"
  print_step_note "更新系统包并安装基础依赖；这是云服务器刚恢复快照后建议执行的第一步。"
  apt_bootstrap
  step_done_msg 1 "Update Ubuntu system and base packages" "$step_start"
  log_timing_since "Ubuntu packages" "$step_start"

  step_start="$(date +%s)"
  step_start_msg 2 "Update OpenClaw and configure gateway"
  print_step_note "云端镜像可能已预装 OpenClaw，这一步会升级到可用版本并配置本机网关。"
  install_openclaw
  step_done_msg 2 "Update OpenClaw and configure gateway" "$step_start"
  log_timing_since "OpenClaw" "$step_start"

  step_start="$(date +%s)"
  step_start_msg 3 "Install and deploy Miloco 2.0"
  print_step_note "下载 Miloco 2.0、安装服务和 OpenClaw 插件，并写入必要 allowlist。"
  install_miloco
  ...
  print_step_note "询问是否绑定个人微信、企业微信或飞书；需要扫码或填写平台凭据时会交给 OpenClaw 官方流程。"
  prompt_openclaw_platform_binding
  ...
  print_step_note "提示米家账号绑定方式；如果需要打开授权链接，会显示后续命令。"
  prompt_mihome_binding
...
run_repair_update() {
  local step_start action_start previous_upgrade
  TOTAL_STEPS=5
  action_start="$(date +%s)"
  previous_upgrade="$RUN_SYSTEM_UPGRADE"
  RUN_SYSTEM_UPGRADE=0
  print_mode_summary "repair"
  ...
  step_start="$(date +%s)"
  step_start_msg 5 "Print next manual actions"
  log_timing_since "Repair/update action" "$action_start"
  step_done_msg 5 "Print next manual actions" "$step_start"
  print_next_actions
  RUN_SYSTEM_UPGRADE="$previous_upgrade"
}
...
dispatch_action() {
  case "$1" in
    ...
    miloco|install-miloco) run_miloco_deploy ;;
    ...
    *)
      die "Unknown INSTALL_ACTION: $1 (use menu|full|openclaw|miloco|repair|restart-openclaw|restart-miloco|weixin|status|logs)"
      ;;
  esac
}
...
main() {
  ...
  case "${1:-}" in
    ...
    --miloco|miloco|install-miloco) INSTALL_ACTION=miloco ;;
