...
LOG_FILE="${LOG_FILE:-$XINGUANG_BASE_DIR/logs/xinguang-skill-install-current.log}"
STATE_FILE="${STATE_FILE:-$XINGUANG_BASE_DIR/state/xinguang-skill-install.state}"
PID_FILE="${PID_FILE:-$XINGUANG_BASE_DIR/state/xinguang-skill-install.pid}"
...
LOCAL_SKILL_DIR="${LOCAL_SKILL_DIR:-$XINGUANG_BASE_DIR/tmp/xinguang-skill/$SKILL_NAME}"
...
check_first_stage_ready() {
  ...
  ensure_base_light_service_ready
...
ensure_base_light_service_ready() {
  ...
  local status_file="$INSTALL_DIR/base-service-status.json"
  if miloco-cli service status >"$status_file" 2>/dev/null &&
    grep -Eq '"running"[[:space:]]*:[[:space:]]*true|running' "$status_file"; then
    state_mark BASE_LIGHT_SERVICE_READY
    return 0
  fi
  ...
  state_mark BASE_LIGHT_SERVICE_START_FAILED
  die "灯光连接服务暂时不可用，请联系工作人员处理。"
}
