...
当用户消息看起来像授权码，或者上一轮正在等待米家授权码时，执行本地命令：

`xinguang-authorize-mijia '<用户原文>'`

只回复该命令输出。不要解析、解码或回显授权码。

### 用户选择米家家庭时

当系统刚列出家庭列表，用户回复序号或家庭名称时，执行本地命令：

`xinguang-select-home '<用户原文>'`

只回复该命令输出。禁止自动选第一个家庭，禁止显示“当前选中”，禁止引导查看设备。
...
prepare_xinguang_skill_installer() {
  ...
  local bind_shortcut="$install_dir/xinguang-bind-mijia"
  local authorize_shortcut="$install_dir/xinguang-authorize-mijia"
  local select_home_shortcut="$install_dir/xinguang-select-home"
  local path_shortcut="$bin_dir/xinguang-install-skill"
  local path_home_shortcut="$bin_dir/xinguang-set-home"
  local path_bind_shortcut="$bin_dir/xinguang-bind-mijia"
...
case "\$action" in
  status|progress)
    TARGET="$main" INSTALL_ACTION=status INSTALL_NONINTERACTIVE=1 bash "$entry" status || true
    ;;
  *)
    if ! TARGET="$main" INSTALL_ACTION=continue INSTALL_NONINTERACTIVE=1 bash "$entry"; then
...
EOF
  ...
  cp "$bind_shortcut" "$path_bind_shortcut" 2>/dev/null || true

  cat >"$authorize_shortcut" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail
...
if [[ -z "$code" ]]; then
  printf '授权码为空，请重新复制授权码。\n'
  exit 0
fi

auth_tmp="$(mktemp "${TMPDIR:-/tmp}/xinguang-authorize.XXXXXX")"
homes_tmp="$(mktemp "${TMPDIR:-/tmp}/xinguang-homes.XXXXXX")"
trap 'rm -f "$auth_tmp" "$homes_tmp"' EXIT

if ! command -v miloco-cli >/dev/null 2>&1; then
  printf '米家账号绑定暂时失败，请联系工作人员处理。\n'
  exit 0
fi

if ! miloco-cli account authorize "$code" >"$auth_tmp" 2>&1; then
  printf '米家账号绑定暂时失败，请重新复制授权码或联系工作人员处理。\n'
  exit 0
fi

if ! miloco-cli scope home list >"$homes_tmp" 2>&1 || [[ ! -s "$homes_tmp" ]]; then
  printf '米家账号绑定成功。\n正在检查家庭列表。\n\n家庭列表暂时无法读取，请联系工作人员处理。\n'
  exit 0
fi

cp "$homes_tmp" "$base/home-list.json" 2>/dev/null || true

python3 - "$homes_tmp" "$base/target-home.env" <<'PY'
...
def collect(obj):
    ...
    if isinstance(obj, dict):
        for key in ("data", "homes", "home_list", "result"):
            val = obj.get(key)
...
homes = []
for item in collect(payload):
    if not isinstance(item, dict):
        continue
    name = item.get("home_name") or item.get("name") or item.get("homeName")
    home_id = item.get("home_id") or item.get("id") or item.get("homeId")
    if name:
        homes.append({"name": str(name), "id": "" if home_id is None else str(home_id)})
...
if not homes:
    ...
    try:
        with open(target_file, "w", encoding="utf-8") as f:
            f.write("XINGUANG_TARGET_HOME=%s\n" % shlex.quote(h["name"]))
            f.write("XINGUANG_TARGET_HOME_ID=%s\n" % shlex.quote(h["id"]))
    except Exception:
        pass
    print("米家账号绑定成功。\n已检测到当前家庭。\n\n下一步：\n请发送「安装馨光 Skill」。")
else:
    lines = ["米家账号绑定成功。", "检测到多个家庭，请选择馨光设备所在家庭：", ""]
    for idx, h in enumerate(homes, 1):
        lines.append(f"{idx}. {h['name']}")
    print("\n".join(lines))
PY
EOF
  chmod +x "$authorize_shortcut"
  cp "$authorize_shortcut" "$path_authorize_shortcut" 2>/dev/null || true

  cat >"$select_home_shortcut" <<'EOF'
#!/usr/bin/env bash
set -Eeuo pipefail

base="$HOME/xinguang-ai-light"
cache="$base/home-list.json"
target="$base/target-home.env"
choice="${*:-}"
choice="${choice//$'\n'/}"
choice="${choice//$'\r'/}"
...
if [[ ! -f "$cache" ]]; then
  printf '家庭列表已过期，请重新发送「绑定米家账号」。\n'
  exit 0
fi

python_out="$(python3 - "$cache" "$choice" "$target" <<'PY'
import json, re, shlex, subprocess, sys

home_file, raw_choice, target_file = sys.argv[1:4]
choice = raw_choice.strip().strip("【】[]")
...
if selected is None:
    print("没有找到这个家庭，请重新输入家庭名称或序号。")
    sys.exit(0)

if selected["id"]:
    res = subprocess.run(["miloco-cli", "scope", "home", "switch", selected["id"]], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if res.returncode != 0:
        print("家庭切换暂时失败，请联系工作人员处理。")
        sys.exit(0)

try:
    with open(target_file, "w", encoding="utf-8") as f:
        f.write("XINGUANG_TARGET_HOME=%s\n" % shlex.quote(selected["name"]))
        f.write("XINGUANG_TARGET_HOME_ID=%s\n" % shlex.quote(selected["id"]))
except Exception:
    pass

print(f"已切换到{selected['name']}。\n\n下一步：\n请发送「安装馨光 Skill」。")
PY
)"
...
exit 0
EOF
  chmod +x "$select_home_shortcut"
  cp "$select_home_shortcut" "$path_select_home_shortcut" 2>/dev/null || true
