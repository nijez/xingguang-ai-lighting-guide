...
7. 真实执行只由工作人员交互式终端开启的一次性真实许可决定。龙虾不得设置环境变量，不得开启真实许可。没有有效真实许可时，执行工具只能 dry-run；有有效真实许可时，执行工具进入真实执行前检查，若灯光服务不可用则阻断，不执行。
8. 禁止调用 `xinguang-test-scene` 作为自然语言链路的替代执行工具。
9. 禁止把预览说成执行结果。
10. 如果本地工具输出包含 `XINGUANG_DIRECT_REPLY_BEGIN` / `XINGUANG_DIRECT_REPLY_END` 标记，最终回复只能使用标记之间的内容，禁止总结、改写、补充、删减、重新组织。
11. 预览工具没有输出 `xg-preview-` 确认编号时，禁止自行生成确认编号。
12. 预览工具返回“为避免误控，请先明确目标设备”时，必须保持拒绝，不得把拒绝改写成灯光预览。
13. 后续 L6-B 执行语句必须携带确认编号，格式为：`执行 xg-preview-xxxx，把门市灯柱调成低亮度暖色，只调这一台。`
14. 后续 L6-B 恢复语句必须携带同一个确认编号，格式为：`恢复 xg-preview-xxxx，把门市灯柱恢复成柔和白光，只调这一台。`
15. 没有 `xg-preview-` 确认编号的控灯语句一律进入预览，不得执行真实控灯。
...
只有 ChatGPT 对抗性审查明确放行 L6-B 后，才允许进入真实自然语言控灯测试。
